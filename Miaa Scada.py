...
# --- FUNCIÓN AUXILIAR PARA GENERAR EL HTML DEL MINI-GRÁFICO ---
def generar_mini_grafico(id_pozo, info_pozo, engine):
    f_fin = datetime.now()
    f_ini = f_fin - timedelta(days=7)
    
    # Solo Caudal y Presión para el popup (para no saturar el espacio)
    tags = {
        'caudal': info_pozo.get('caudal'),
        'presion': info_pozo.get('presion')
    }
    tags = {k: v for k, v in tags.items() if v and v != 'N/A'}
    
    if not tags:
        return "<p style='color:gray; font-size:10px;'>Sin datos de telemetría configurados.</p>"
    ...
    try:
        query = f"""
            ...
            FROM vfitagnumhistory h
            JOIN VfiTagRef r ON h.GATEID = r.GATEID
            WHERE r.NAME IN ('{"', '".join(tags.values())}') 
        ...
        if df.empty:
            return "<p style='color:gray; font-size:10px;'>Sin datos en los últimos 7 días.</p>"

        # Crear un gráfico pequeño y estético
        fig = px.line(df, x='FECHA', y='VALUE', color='TagName', template="plotly_dark")
        
        fig.update_layout(
            margin=dict(l=0, r=0, t=0, b=0),
            height=150, # Tamaño compacto
            showlegend=False,
